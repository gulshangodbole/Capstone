        ...
        >
            ...
            {pathname === '/admin' ?
                ...
                >
                    ...
                    {isAuth ? <h1 style={{cursor: 'default'}} onFocus={(e) => e.target.style.cursor = "pointer"}
                                  onClick={() => {
                                      navigate('/profile')
                                  }}>{currentUser.fullname}</h1> : <Link to="/login">Login</Link>}


                    {isAuth ? <Link to="/login"> <Button onClick={() => dispatch({type: LOGIN_FAILURE})}>Log
                        Out</Button></Link> : <Link to="/signup"><Button
