        ...
        <Box mb={8}>
            {loading ? (
                ...
                <Table variant="striped" colorScheme="purple">
                    <TableCaption fontFamily={"Archivoblack"} fontSize={"35px"} placement="top">Loan
                        Details</TableCaption>
                    <Thead>
                        <Tr>
                            <Th>Loan ID</Th>
                            <Th>Loan Amount</Th>
                        ...
                        {loans
                            ...
                            .map((loan) => (
                                <Tr key={loan.id}>
                                    ...
                                    <Td>{loan.loanTerm}</Td>
                                    <Td>{loan.status}</Td>
                                    <Td><Button isDisabled={loan.status.toLowerCase() !== 'accepted'}
                                                onClick={() => navigate('/payment', {state: loan.id})}>Pay Loan</Button></Td>
                                    <Td><Button onClick={() => navigate('/paymentHistory', {state: loan.id})}>Past
                                        Payments</Button></Td>
                                </Tr>
                            ))}
                    </Tbody>
